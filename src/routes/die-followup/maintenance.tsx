...
function KaliplarPage() {
  ...
  // Hazır olma saati (ör. 10:00): o gün o saatten önce plan kalıbı kullanmaz.
  ...
  async function submitReadiness(ready: boolean) {
    ...
    const ok = await setReadiness({
      ...
      readyMinute:
        ready || !readyDate || !/^\d{1,2}:\d{2}$/.test(readyTime)
          ? undefined
          : Number(readyTime.split(':')[0]) * 60 + Number(readyTime.split(':')[1]),
  ...
  return (
    <div className="w-full px-4 py-6 sm:px-6 sm:py-8">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        A die that is not ready is not planned at all. Give the date (and, if
        known, the time) it will be ready and the plan resumes from that
        moment; leave the date empty and it stays out of the plan until
        someone releases it. If the customer needs parts before that, the die
        shows as critical on the PlanningExpert Alarms page.
      </p>
              ...
              {heldMolds.map((r) => (
                <tr key={r._id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2 text-muted-foreground">
                    {r.readyDate ? (
                      `${r.readyDate}${
                        r.readyMinute !== undefined
                          ? ` ${String(Math.floor(r.readyMinute / 60)).padStart(2, '0')}:${String(r.readyMinute % 60).padStart(2, '0')}`
                          : ''
                      }`
                    ) : (
